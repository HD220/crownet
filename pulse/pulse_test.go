...
func TestNewPulse(t *testing.T) {
	...
	if p.EmittingNeuronID != emitterID { t.Errorf("NewPulse EmittingNeuronID got %d, want %d", p.EmittingNeuronID, emitterID) }
	if p.OriginPosition != origin { t.Errorf("NewPulse OriginPosition got %v, want %v", p.OriginPosition, origin) }
	if p.BaseSignalValue != signal { t.Errorf("NewPulse BaseSignalValue got %f, want %f", p.BaseSignalValue, signal) }
	if p.CreationCycle != creationCycle { t.Errorf("NewPulse CreationCycle got %d, want %d", p.CreationCycle, creationCycle) }
	if p.MaxTravelRadius != maxRadius { t.Errorf("NewPulse MaxTravelRadius got %f, want %f", p.MaxTravelRadius, maxRadius) }
	if p.CurrentDistance != 0.0 { t.Errorf("NewPulse CurrentDistance got %f, want 0.0", p.CurrentDistance) }
}
...
func TestPulse_GetEffectShellForCycle(t *testing.T) {
	...
	if s != 0.0 || e != 0.0 { t.Errorf("Initial shell: got (%f,%f), want (0,0)",s,e)}
...
func TestPulseList_AddClearCount(t *testing.T) {
	pl := NewPulseList()
	if pl.Count() != 0 { t.Errorf("New PulseList count = %d, want 0", pl.Count())}

	p1 := New(0, common.Point{}, 1.0, 0, 10)
	pl.Add(p1)
	if pl.Count() != 1 { t.Errorf("After Add(p1), count = %d, want 1", pl.Count())}

	pl.Add(nil) // Should not add nil
	if pl.Count() != 1 { t.Errorf("After Add(nil), count = %d, want 1", pl.Count())}

	p2 := New(1, common.Point{}, 1.0, 0, 10)
	p3 := New(2, common.Point{}, 1.0, 0, 10)
	pl.AddAll([]*Pulse{p2, nil, p3}) // Add a slice with a nil
	if pl.Count() != 3 { t.Errorf("After AddAll({p2,nil,p3}), count = %d, want 3", pl.Count())}

	all := pl.GetAll()
	if len(all) != 3 {t.Errorf("GetAll() len = %d, want 3", len(all))}
...
func TestPulseList_ProcessCycle(t *testing.T) {
	...
	if pl.Count() != 2 { t.Errorf("Cycle 2: After adding new, expected 2 active pulses, got %d", pl.Count())}
