...
**Branch Git Proposta:** (Não aplicável, tarefa de análise)
...
**Notas/Decisões:**
- A análise pode envolver a geração de flame graphs ou outros tipos de visualizações que o `pprof` oferece.
- O objetivo não é implementar otimizações nesta tarefa, mas sim identificar onde elas podem ser necessárias.
- Comparar os perfis dos dois modos pode revelar se os gargalos são comuns ou específicos de cada modo de operação.
