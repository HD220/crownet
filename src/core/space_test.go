...
func TestDistanceEuclidean(t *testing.T) {
	p1 := [SpaceDimensions]float64{1, 2, 3} // Preenchendo apenas as 3 primeiras para simplificar o teste
	p2 := [SpaceDimensions]float64{4, 5, 6} // Restantes são 0.0

	// Distância = sqrt((4-1)^2 + (5-2)^2 + (6-3)^2)
	// = sqrt(3^2 + 3^2 + 3^2) = sqrt(9 + 9 + 9) = sqrt(27)
...
func TestNormalizeVector(t *testing.T) {
	v1 := [SpaceDimensions]float64{3, 4} // Magnitude 5
...
func TestClampPosition(t *testing.T) {
	...
	// Test case 1: Position within bounds
	...
	// Test case 2: Position with some negative values
	...
	// Test case 3: All values out of bounds
	...
	// Need to compare element by element for full array
	for i := 0; i < SpaceDimensions; i++ {
